...
import { diff, commitRoot } from './diff/diff.mjs';
...
function render(vnode, remoteNode) {
  ...
  (_options$_root = options._root) === null || _options$_root === void 0 ? void 0 : _options$_root.call(options, vnode, remoteNode); // To be able to support calling `render()` multiple times on the same
  ...
  diff(remoteNode, getRemoteRoot(remoteNode), newVNode, oldVNode, {}, unaccountedForChildren !== null && unaccountedForChildren !== void 0 && unaccountedForChildren.length ? [...unaccountedForChildren] : null, commitQueue, null); // Flush all queued effects
