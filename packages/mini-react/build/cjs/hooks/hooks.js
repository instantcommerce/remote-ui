...
const oldBeforeRender = options["default"]._render;
const oldAfterDiff = options["default"].diffed;
const oldCommit = options["default"]._commit;
const oldBeforeUnmount = options["default"].unmount;
const RAF_TIMEOUT = 100;
let prevRaf;

options["default"]._render = vnode => {
...
options["default"]._commit = (vnode, commitQueue) => {
...
function getHookState(index, type) {
  ...
  (_options$_hook = options["default"]._hook) === null || _options$_hook === void 0 ? void 0 : _options$_hook.call(options["default"], currentComponent, index, currentHook || type);
...
function useEffect(effect, inputs) {
  ...
  if (!options["default"]._skipEffects && argsChanged(state._args, inputs !== null && inputs !== void 0 ? inputs : [])) {
    state._value = effect;
    state._args = inputs;
...
function useDebugValue(value, formatter) {
  ...
  (_options$useDebugValu = options["default"].useDebugValue) === null || _options$useDebugValu === void 0 ? void 0 : _options$useDebugValu.call(options["default"], formatter ? formatter(value) : value);
}
/**
...
function afterPaint(newQueueLength) {
  if (newQueueLength === 1 || prevRaf !== options["default"].requestAnimationFrame) {
    prevRaf = options["default"].requestAnimationFrame;
